...
from .runtime import get_entry_data, get_product_input_value, set_product_input_value
...
def get_product_input_entity(
    ...
    entity = get_entry_data(hass, entry_id).get(DATA_PRODUCT_INPUT_ENTITY)
...
class GrocyProductInput(TextEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        get_entry_data(self.hass, self._entry.entry_id)[
            DATA_PRODUCT_INPUT_ENTITY
        ] = self
        self.async_on_remove(self._async_unregister_entity)

    def _async_unregister_entity(self) -> None:
        domain_data = self.hass.data.get("grocy_ai_assistant", {})
