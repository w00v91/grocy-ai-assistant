...
from .runtime_state import (
    ...
    RUNTIME_RESPONSE,
    async_record_response_timing,
    async_set_product_input_value,
    async_set_runtime_payload,
    ensure_entry_runtime_data,
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    entry_runtime = ensure_entry_runtime_data(hass, entry.entry_id)
    entry_runtime.clear()
    entry_runtime[DATA_ENTRY_CONFIG] = _migrate_entry_payload(
        {**entry.data, **entry.options}
    )
    ensure_entry_runtime_state(hass, entry.entry_id)
    ...
    def _set_response_timing_states(duration_ms: float) -> None:
        async_record_response_timing(hass, entry.entry_id, duration_ms)
