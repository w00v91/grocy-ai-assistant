...
def get_entry_runtime_data(hass: HomeAssistant, entry_id: str) -> dict[str, Any]:
    """Return the runtime data bucket for a config entry."""
...
def get_entry_entity_runtime(hass: HomeAssistant, entry_id: str) -> RuntimePayload:
    """Return the entity runtime payload for a config entry."""
    runtime = get_entry_runtime_data(hass, entry_id).setdefault(
        DATA_ENTRY_RUNTIME,
...
def async_set_runtime_sensor_payload(
    ...
    *,
    ...
    get_entry_entity_runtime(hass, entry_id)[key] = payload
...
def get_runtime_sensor_payload(
    ...
    payload = get_entry_entity_runtime(hass, entry_id).get(key)
...
def get_product_input_value(hass: HomeAssistant, entry_id: str) -> str:
    ...
    payload = get_entry_entity_runtime(hass, entry_id).get(RUNTIME_PRODUCT_INPUT)
