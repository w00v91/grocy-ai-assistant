...
def test_native_shopping_removes_extra_product_search_hero_bar():
    source = (
        Path(__file__).resolve().parents[2]
        / "grocy_ai_assistant"
        / "custom_components"
    ...
    assert "heroTitle.textContent = 'Produktsuche';" not in source
    assert "root.append(hero, searchBar, listSection);" not in source
    assert "root.append(searchBar, listSection);" in source
