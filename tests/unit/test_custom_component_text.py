...
def test_text_entity_unregister_removes_registered_reference():
    hass = _FakeHass()
    entity = text_module.GrocyProductInput(_FakeEntry())
    entity.hass = hass
    asyncio.run(entity.async_added_to_hass())

    entity._async_unregister_entity()

    assert text_module.get_product_input_entity(hass, "entry-1") is None
